...
class CtrlSpace extends Ctrl
{
	...
	public static function actionSpaceEdit()
	{
		...
		if(Req::isParam("formValidate"))
		{
			...
			$curObj=$curObj->createUpdate("name=".Db::formatParam("name").", description=".Db::formatParam("description").", public=".Db::formatParam("public").", password=".Db::formatParam("password").", usersInscription=".Db::formatParam("usersInscription").", usersInvitation=".Db::formatParam("usersInvitation").", wallpaper=".Db::formatParam("wallpaper"));
			...
			if($curObj->isNew() && in_array("calendar",Req::getParam("moduleList")) && Req::isParam("addSpaceCalendar")){
				$newCalendar=new MdlCalendar(0);
				$newCalendar=$newCalendar->createUpdate("title=".Db::format($curObj->name).", type='ressource', evtColorDisplay='background'");
