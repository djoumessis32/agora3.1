...
{
	confirmRedir("<?= Txt::trad("confirmDelete") ?>", "?ctrl=<?= Req::$curCtrl ?>&action=<?= Req::$curAction ?>&deleteCustomWallpaper="+$("select[name='wallpaper']").val());
}
</script>
...
select[name='wallpaper']		{height:90px; min-width:80px; max-width:200px;}
...
<div id="wallpaperMain">
	<div id="wallpaperSelect">
		<select name="wallpaper" multiple="multiple">
			<?php
			//"Par défaut" / "Ajouter"
			if(Req::$curCtrl=="agora")	{echo "<option value='add'>".Txt::trad("ajouter")."</option>";}
			else						{echo "<option value='' ".(empty($curWallpaper)?"selected":null).">".Txt::trad("par_defaut")."</option>";}
	...
	<div id="wallpaperAdd"><input type="file" name="wallpaperFile"></div>
	<?php if(Req::$curCtrl=="agora"){ ?>
	<div id="wallpaperDelete" class="sLink" onclick="wallpaperDelete()"><img src="app/img/delete.png"><?= Txt::trad("AGORA_suppr_wallpaper") ?></div>
	<?php } ?>
</div>